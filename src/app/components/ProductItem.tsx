...
function ProductItem({product}:{product: Product}) {
    ...
    return (
        <div className="card m-2">
            <div className="card-img flex items-center justify-center overflow-hidden">
                <span><img src={product.image_url} alt="" className="object-cover h-full w-full "/></span>
            </div>
            <div className="card-info">
                <div className="flex justify-between">
                    <p className="text-title">{product.name}</p>
                    {product.own_product && <p className="text-body font-mono ml-3 text-bold text-gray-500">(Own Product)</p>}
                </div>
            </div>
            <div className="card-footer">
                <div className="justify-items-stretch h-20">
                    <p className="text-body text-gray-500">{product.condition == "baru" ? "Baru" : "Bekas"}</p>
                    <p className="text-title">${product.price}</p>
                </div>
                <Link href={`/products/${product.id}`} className={product.own_product ? "pointer-events-none card-button" : "card-button"}>
