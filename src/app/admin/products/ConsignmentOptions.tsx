...
import EditProductDrawer from "@/app/admin/products/EditProductDrawer";
import { EdgeStoreProvider } from '@/app/account/consignment/EdgeStore';
import {deleteProduct} from "@/app/admin/AdminActions";
        ...
        <Popover>
            ...
            <PopoverContent className="flex justify-center pl-9 rounded-2xl items-center w-[258px]">
                <div className={"w-full"}>
                    ...
                    {product.stock == 0 &&
                    <button className={"bt w-10/12 h-12 mt-3"} onClick={() => deleteProduct(product.id)}>Delete</button>}
                </div>
            </PopoverContent>
        </Popover>
    )
