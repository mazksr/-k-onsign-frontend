...
export async function inputTrackingNumber(prevState: any, formData: FormData) {
    const tracking_numberEntry = formData.get("tracking_number");
    const id = formData.get("id");
    const carrier_company = formData.get("carrier_company");

    const tracking_number = typeof tracking_numberEntry === 'string' && typeof carrier_company === "string" ? `${tracking_numberEntry} (${carrier_company})` : "";
